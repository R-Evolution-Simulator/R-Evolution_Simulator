...
class BaseDiagramCanvasFrame(BaseFrame):
    def __init__(self, father, sim_name, subject, subplots=1):
        ...
        self.WIDGETS = {'canvas': (FigureCanvasTkAgg, {'figure': self.figure, 'master': self}, {'fill': tk.BOTH, 'expand': True})}
        self.father = father
...
class DiagramCommandBar(BaseFrame):
    ...
    def __init__(self, father, windows):
        self.WIDGETS = {'follow_play': (tk.Checkbutton, {'text': "Follow play", 'command': windows[0].change_follow_play}, {'side': tk.LEFT}),
                        'tick_difference': (tk.Spinbox, {'from_': self.TICK_DIFFERENCE_FROM, 'to': windows[1].max_tick, 'increment': self.TICK_DIFFERENCE_INCREMENT, 'command': windows[0].tick_difference_set}, {'side': tk.LEFT}),
...
class SpreadDiagram(BaseDiagramCanvasFrame):
    ...
    def set_subplot(self):
        ...
        for i in range(1, 8):
            self.subplots[0].fill_between(self.data[0], self.data[(i + 1) * 2], self.data[i * 2], color=self.COLOURS[color][i], label=self.LABELS[i])
        self.subplots[1].fill_between(self.data[0], self.data[17], color=self.COLOURS[color][0], label=self.LABELS[0])
        for i in range(17, 24):
            self.subplots[1].fill_between(self.data[0], self.data[i + 1], self.data[i], color=self.COLOURS[color][i - 16], label=self.LABELS[i - 16])
